function Invoke-TehtrisTest {
    ...
    Param(
        ...
        [Parameter(Mandatory = $false,
            ParameterSetName = 'technique')]
        [String]
        $PathToAtomicsFolder = $( if ($IsLinux -or $IsMacOS) { $Env:HOME + "/AtomicRedTeam/tehtris" } else { $env:HOMEDRIVE + "\AtomicRedTeam\tehtris" }),
        ...
        [Parameter(Mandatory = $false,
            ParameterSetName = 'technique')]
        [String]
        $ExecutionLogPath = $( if ($IsLinux -or $IsMacOS) { "/tmp/Invoke-TehtrisTest-ExecutionLog.csv" } else { "$env:TEMP\Invoke-TehtrisTest-ExecutionLog.csv" }),
